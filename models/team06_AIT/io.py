...
def _load_config(model_dir):
    cfg_path = os.path.join(model_dir, "team06_ait.json")
...
def main(model_dir, input_path, output_path, device):
    ...
    tmp_output = os.path.join(output_path, "_seesr_tmp")
    ...
    if os.path.isdir(sample0):
        src_files = sorted(glob.glob(os.path.join(sample0, "*.png")))
    else:
        src_files = sorted(glob.glob(os.path.join(tmp_output, "**", "*.png"), recursive=True))
